...
// ============================================
// Generic Crosswalk Compiler (Any Source)
// ============================================

/**
 * Compile AST with crosswalk expansion, preserving source-specific syntax.
 * This is used for domain-specific crosswalks (e.g., dry eye vocabulary)
 * that apply to ALL sources, not just ERIC/DOAJ.
 */
function compileWithCrosswalk(
  ast: ASTNode,
  source: string,
  crosswalk?: { expand(term: string, region?: string): string[] },
  region?: string,
): string {
  switch (ast.type) {
    case "term": {
      if (crosswalk) {
        const expanded = crosswalk.expand(ast.value, region);
        if (expanded.length > 1) {
          // Use OR expansion for domain terms
          switch (source) {
            case "pubmed":
              return `(${expanded.map(t => `"${t}"[All Fields]`).join(" OR ")})`;
            case "openalex":
      ...
      // Fallback to source-specific syntax
      switch (source) {
        case "pubmed":
          return `"${ast.value}"[All Fields]`;
        case "openalex":
    ...
    case "and": {
      const left = compileWithCrosswalk(ast.left, source, crosswalk, region);
      const right = compileWithCrosswalk(ast.right, source, crosswalk, region);
      switch (source) {
        case "pubmed":
    ...
    case "not": {
      const operand = compileWithCrosswalk(ast.operand, source, crosswalk, region);
      switch (source) {
        case "pubmed":
          return `NOT ${operand}`;
        default:
...
// Crosswalk compilers for sources that support domain-specific expansion
export const CROSSWALK_COMPILERS: Record<string, CrosswalkCompiler> = {
  eric: compileERIC,
  doaj: compileDOAJ,
  pubmed: (ast, crosswalk, region) => compileWithCrosswalk(ast, "pubmed", crosswalk, region),
  openalex: (ast, crosswalk, region) => compileWithCrosswalk(ast, "openalex", crosswalk, region),
  semantic_scholar: (ast, crosswalk, region) => compileWithCrosswalk(ast, "semantic_scholar", crosswalk, region),
  crossref: (ast, crosswalk, region) => compileWithCrosswalk(ast, "crossref", crosswalk, region),
  core: (ast, crosswalk, region) => compileWithCrosswalk(ast, "core", crosswalk, region),
  scopus: (ast, crosswalk, region) => compileWithCrosswalk(ast, "scopus", crosswalk, region),
};
